...
void local_voice_task(void *arg)
{
    ...
    while (1)
    {
        ...
        /* Push to talk */
        if (g_SW1Pressed == true && s_asrSession == ASR_SESSION_WAKE_WORD && appAsrShellCommands.ptt == ASR_PTT_ON)
        {
            g_SW1Pressed             = false;
            g_sampleCount = 0;
        ...
        {
            if (VIT_DetectionResults == VIT_WW_DETECTED  && appAsrShellCommands.ptt == ASR_PTT_OFF)
            {
