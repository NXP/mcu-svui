    ...
    else
    {
        ...
        else
        {
...
#endif
            {
                ...
                {
                    uint16_t current_lang_id = get_language_id_from_str(argv[idx]);

                    if ((current_lang_id == UNDEFINED_LANGUAGE) ||
                        (activeLanguage & current_lang_id))
                    {
                        configPRINTF(("\r\nERROR(arg: %s) ", argv[idx]));
                        status = kStatus_SHELL_Error;
                        break;
                    }
                    else
                ...
                if (status == kStatus_SHELL_Success)
                {
                    ...
                    else
                    {
                        ...
                        oob_demo_control.language = get_language_id_from_str(argv[1]);
                    }
...
void sln_shell_task(void *arg)
{
    ...
    while (1)
    {
        ...
        {
            SHELL_Printf(s_shellHandle, "Firmware version: %d.%d.%d, Current Bank: %s\r\n", APP_MAJ_VER, APP_MIN_VER,
